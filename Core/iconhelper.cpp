...
QPixmap IconHelper::draw(IconGlyph glyph, const QSize &size, const QColor &color)
{
    ...
    case IconGlyph::Minus:
        drawMinus(painter, rect);
        break;
    case IconGlyph::Maximize:
        drawMaximize(painter, rect);
        break;
    case IconGlyph::Close:
        drawClose(painter, rect);
        break;
    case IconGlyph::ChevronRight:
        drawChevronRight(painter, rect);
        break;
    }
...
void IconHelper::drawMinus(QPainter &painter, const QRectF &rect)
{
    painter.save();
    QPen pen = painter.pen();
    pen.setWidthF(rect.height() * 0.14);
    painter.setPen(pen);
    painter.drawLine(QPointF(rect.left() + rect.width() * 0.2, rect.center().y()),
                     QPointF(rect.right() - rect.width() * 0.2, rect.center().y()));
    painter.restore();
}

void IconHelper::drawMaximize(QPainter &painter, const QRectF &rect)
{
    painter.save();
    qreal radius = rect.width() * 0.18;
    QRectF box(rect.center().x() - radius, rect.center().y() - radius, radius * 2, radius * 2);
    painter.drawRoundedRect(box, radius * 0.45, radius * 0.45);
    painter.restore();
}

void IconHelper::drawClose(QPainter &painter, const QRectF &rect)
{
    painter.save();
...
void IconHelper::drawChevronRight(QPainter &painter, const QRectF &rect)
{
    painter.save();
    QPointF top(rect.left() + rect.width() * 0.35, rect.top() + rect.height() * 0.25);
    QPointF mid(rect.right() - rect.width() * 0.25, rect.center().y());
    QPointF bottom(rect.left() + rect.width() * 0.35, rect.bottom() - rect.height() * 0.25);
